...
you MUST load brand-identity ($ sis-streamlit -> brand-identity) before generating any chart, color, or label code.
brand-identity is the ONLY source of truth for chart type selection, colors, axis format strings, labelAngle, legend placement, and axis title conventions.
do NOT hardcode any color hex value, format string, or axis title text - get them from brand-identity, every time.
...
you MUST load brand-identity ($ sis-streamlit -> brand-identity) before generating any chart code.
brand-identity is the ONLY source of truth for colors, axis format strings, labelAngle, legend placement, and axis title conventions. do NOT hardcode color hex values, format strings, or axis title text without consulting it.
...
- renewal rate trend over time (line with points, adaptive granularity: day/week/month)
  ...
  query: `SELECT DATE_TRUNC('month', renewal_date) AS period, SUM(is_renewed)*1.0/COUNT(*) AS renewal_rate FROM FACT_RENEWAL [+ filters] GROUP BY period ORDER BY period`
  chart: line with points. encodings: X = period (time, title=None mandatory for time axes). Y = renewal_rate (percentage).
  colors, format strings, and axis titles: see brand-identity.
- renewal rate by region (vertical bar, sorted descending)
  X = region code (short 2-letter codes - use vertical/column orientation)
  chart: vertical bar. encodings: X = region (sorted descending by rate). Y = renewal_rate (percentage).
  colors, format strings, labelAngle, and axis titles: see brand-identity.
- renewal_outcome distribution by segment (proportional 100% stacked horizontal bar - shows % share of each outcome per segment)
  ...
  structural encoding (visual style - colors, format strings, label limits, legend - you MUST get from brand-identity):
    alt.X("n:Q", stack="normalize")
    alt.Y("segment:N", sort=alt.EncodingSortField("n", op="sum", order="descending"))
    alt.Color("renewal_outcome:N", domain=["RENEWED","LAPSED","NOT_TAKEN_UP","CANCELLED"])
      <- domain order is REQUIRED. colors for each outcome come from brand-identity primary/accent palette.
    alt.Order("renewal_outcome:N", sort="descending") <- REQUIRED: puts RENEWED at the 0% left edge
    tooltip: renewal_outcome, pct (percentage), n (count)
...
- average premium_change_pct by price_shock_band (vertical bar, sorted by band order: 0_TO_5, 5_TO_10, 10_TO_15, GT_15)
  ...
  chart: vertical bar. encodings: X = price_shock_band (sorted by band order). Y = avg_premium_change (percentage).
  colors, format strings, and axis titles: see brand-identity.
- average premium_change_pct by renewal_outcome (vertical bar: all 4 outcomes - RENEWED / LAPSED / NOT_TAKEN_UP / CANCELLED)
  do NOT apply `is_final_offer = 1` to this chart. LAPSED/NOT_TAKEN_UP/CANCELLED policies often have no final offer event,
  ...
  you MUST join to FACT_RENEWAL to get renewal_outcome from the authoritative source (FACT_RENEWAL always has all outcomes populated).
  ...
  chart: vertical bar. encodings: X = renewal_outcome. Y = avg_change (percentage).
  colors, format strings, labelAngle, and axis titles: see brand-identity.
